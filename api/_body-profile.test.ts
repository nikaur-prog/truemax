...
test("device migration no longer calls the historical empty-row RPC", () => {
  ...
  assert.match(put, /if \(parsed\.source === "device_migration"\) \{[\s\S]*?ignoreDuplicates: true[\s\S]*?return json\(await state\(user\.id\)\);/);
  assert.doesNotMatch(put, /\.rpc\(/, "the applied RPC remains untouched but is no longer a route dependency");
