...
function cuesFor(timed: TimedBeat[]): SfxCue[] {
  ...
  for (const b of timed) {
    // No keystroke cues. The rolling caption they accompanied is gone — the
    // frame prints nothing to type — and ticks with no glyphs are noise under
    // the voice. The click stays: it marks the measurement landing, which
    // still happens.
