...
const RECIPES: Record<string, (p: SidePoints, m: ScoredMetric, span: number) => Prim[]> = {
  ...
  facialConvexity: (p, m) => [{ kind: "angle", v: p.glabella, a: p.subnasale, b: p.pogonion, label: fmt(m) }],
  totalFacialConvexity: (p, m) => [{ kind: "angle", v: p.glabella, a: p.pronasale, b: p.pogonion, label: fmt(m) }],
  nasofrontalAngle: (p, m) => [{ kind: "angle", v: p.glabella, a: p.nasion, b: p.pronasale, label: fmt(m) }],
