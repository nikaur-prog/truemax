...
- Browser: a JPEG side copy plus automatic and corrected point coordinates.
- Server route: `POST /api/side-correction-feedback`, requiring a valid Supabase
  access token and rejecting cross-origin, oversized or malformed submissions.
...
- Database: `public.side_landmark_feedback`, inaccessible to `anon` and
...
- Retention: metadata and photo expire after 90 days. The daily protected cron
  removes expired objects and rows. Account deletion queues its associated
  storage objects for the same cleanup route.

The schema is in
`supabase/migrations/20260812034351_side_correction_feedback.sql` and has been
applied to project `ruvgkrlfmixfnmnzqgap`.
...
At 12 August 2026, `SUPABASE_SECRET_KEY` is the only missing variable for this
feature. The route is intentionally not deploy-ready until it exists.
...
- Run the cleanup route with Vercel Cron authentication and confirm an expired
  test row removes both its object and metadata.
